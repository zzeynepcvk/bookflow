...
const BookCard: React.FC<Props> = ({ book, onDelete, onUpdate, onOpenNotes }) => {
  ...
  const inc = (n: number) => {
    if (!total) return // sayfa sayısı yoksa ilerleme yapma
    const next = Math.max(0, Math.min(total, book.readPages + n))
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            ...
            <div className="text-sm text-gray-400">
              {book.addedAt ? new Date(book.addedAt).toLocaleDateString() : ""}
            </div>
          </div>
